...
#if 0 // SEB A SUP
static void ctrlapi_test_can(AFB_ReqT req)
{
    int rc;
    AFB_EventT evt = AFB_EventMake(req->dynapi, "can0");
    if (!AFB_EventIsValid(evt)) {
        AFB_ReqFailF(req, "ERROR", "evt=???"); // %d", AFB_EventIsValid(evt));
        return;
    }

    rc = AFB_ReqSubscribe(req, evt);
    if (rc < 0) {
        AFB_ReqFailF(req, "ERROR", "sub rc=%d", rc);
        return;
    }

    rc = AFB_EventPush(evt, json_object_get(AFB_ReqJson(req)));
    if (rc < 0) {
        AFB_ReqFailF(req, "ERROR", "push rc=%d", rc);
        return;
    }

    AFB_ReqSuccess(req, json_object_new_string("OK"), NULL);
}
#endif
...
// SEB    afb_dynapi_on_init(apiHandle, CtrlInitOneApi);
